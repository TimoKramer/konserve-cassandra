(ns konserve-cassandra.core
  (:require [clojure.core.async :refer [<!! thread put! close! chan]]
...
(defrecord CassandraStore [conn default-serializer serializers compressor encryptor read-handlers write-handlers locks]
        ...
        (try
          (let [[fkey & rkey] key-vec
            ...
            (if (first old-val)
              ...
              (io/insert-both conn (str-uuid fkey) [(.toByteArray mbaos) (.toByteArray vbaos)]))
  ...
  ;    (thread
  ;      (try
  ;        (let [[[mheader old-meta'] [_ old-val]] (io/get-all conn (str-uuid key))
  ...
  ;            (map #(put! res-ch %) keys))
  ;          (close! res-ch))
  ;        (catch Exception e (put! res-ch (prep-ex "Failed to retrieve keys from store" e))))
  ;      res-ch)))
...
(defn delete-store [connection]
  (let [res-ch (chan 1)]
    (thread
      (try
        (io/drop-table connection)
        (close! res-ch)
...
(comment
  ...
  (def store (<!! (new-cassandra-store config)))
